...
const orderBalancer = new LoadBalancer(['http://localhost:4003']);
...
module.exports = (app) => {
    ...
    // Redirection vers le service produit
    app.use(
        '/product', protect,
        createProxyMiddleware({
    ...
    // Redirection vers le service Commande
    app.use(
        '/order', protect,
        createProxyMiddleware({
            target: orderBalancer.getNextServer(),
            changeOrigin: true,
            router: () => orderBalancer.getNextServer(),
            on: {
                proxyReq: (proxyReq, req) => {
                    // Ajouter les en-têtes personnalisés à la requête envoyée au microservice
                    if (req.headers['x-user-id']) {
